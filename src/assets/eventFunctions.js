...
export function isDisplay(type, position, lev) {
  let pos = position;
  pos++;
  for (let i = pos; i < inputs.length; i++) {
    const comparingLevel = inputs[i].getAttribute('data-level');
...
export function indeterminatingDads(boolean, initialPosition, lev) {
  const fathers = [...inputs].splice(0, initialPosition);
  const fathersIndex = [];

  for (let j = initialPosition - 1; j >= 0; j--) {
    const comparingLevel = fathers[j].getAttribute('data-level');
    const itemPosition = fathers[j].getAttribute('data-position');
    // verificar se já há um elemento de tal nível como indeterminate,
    // para evitar que elementos 'irmãos' tenham indeterminate
    if (comparingLevel < lev) {
      if (
        fathersIndex.find((index) => index === comparingLevel) === undefined
      ) {
...
export function isChecked(boolean, position, lev) {
  const initialPosition = inputs[position].getAttribute('data-position');
  let pos = position;
  pos++;
  let never = true;
  for (let i = pos; i < inputs.length; i++) {
    const comparingLevel = inputs[i].getAttribute('data-level');
    const itemPosition = inputs[i].getAttribute('data-position');
